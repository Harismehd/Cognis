...
# Static fallback messages — only used as last-resort fallback, not as training signal.
# The real messages are built dynamically from code context in _build_training_message().
ERROR_MESSAGES = {
    ...
    "concept_gap": "This isn't a typo — it looks like the logic doesn't match the intended task. Let's step back.",
...
def _get_line_content(code: str, line_number: int) -> str:
    """Extract a specific line from code for use in context-aware messages."""
    lines = code.splitlines()
    idx = line_number - 1
    if 0 <= idx < len(lines):
        return lines[idx].strip()[:60]
    return ""
...
def _build_training_message(error_type: str, code: str, hint_style: str) -> str:
    """
    Build a context-aware, student-facing training message that references the actual code.
    This is what the model learns to produce — specific, Socratic, code-grounded.
    """
    line_number = len(code.splitlines()) if code else 1
    line_content = _get_line_content(code, line_number)
    line_ref = f'`{line_content}`' if line_content else "this line"

    bases = {
        "missing_colon": f"{line_ref} starts a block — what symbol must end a block header in Python?",
        "spelling_error": f"{line_ref} — does Python recognize that keyword? Check each character carefully.",
        "missing_parenthesis": f"Count every `(` and `)` on {line_ref}. Do they pair up?",
        "missing_bracket": f"Count every `[` and `]` on {line_ref}. Does every opener have a closer?",
        "missing_quote": f"Check the quotes on {line_ref}. Every string needs both an opening and a closing quote.",
        "indentation_error": f"{line_ref} — is this line's indentation consistent with the block above it?",
        "unexpected_indent": f"{line_ref} is indented, but Python didn't expect a new block here. Does the line above end with a colon?",
        "undefined_variable": f"You used a name on {line_ref} — was it assigned a value before this point?",
        "wrong_operator": f"{line_ref} — is that the right operator for what you're comparing or calculating?",
        "comparison_vs_assignment": f"Inside a condition on {line_ref}, `=` assigns — did you mean `==` to compare?",
        "type_confusion": f"Look at {line_ref} — what type is on each side? Can a string and a number work together like that?",
        "index_error": f"On {line_ref}, check the index. How many items are in the list? Python counts from 0.",
        "infinite_loop_risk": f"Look at {line_ref} — what value changes inside this loop so it can eventually stop?",
        "off_by_one_error": f"Check the boundary on {line_ref}. Is the range one step too many or too few?",
        "wrong_loop_condition": f"On {line_ref} — will this condition stop the loop at the right moment?",
        "wrong_formula": f"Trace {line_ref} step by step. Is the order of operations what you intend?",
        "concept_gap": f"Step back from {line_ref}. In plain English, what should this line do? Does your code do that?",
        "multiple_errors": f"There's more than one issue around {line_ref}. Fix the first error completely before looking at the rest.",
        "unknown_ambiguous": "Keep typing — the pattern isn't clear enough yet. I'll help when it is.",
        "incomplete_statement": f"{line_ref} — what should come after this keyword? The statement isn't complete yet.",
        "no_error": "Code looks good so far — keep going.",
    }

    base = bases.get(error_type, ERROR_MESSAGES.get(error_type, f"Review {line_ref} carefully."))
    ...
    return f"{prefixes.get(hint_style, '')}{base}"
...
def _make_target(error_type: str, line: int, column: int, message: str, intervention_type: str,
                 should_speak: bool, tone: str, next_best_action: str,
                 confidence: float, error_risk: float) -> Dict[str, Any]:
    diagnosis = [{
...
def generate_code(error_type: str) -> Tuple[str, str, str, str]:
    ...
    if error_type == "spelling_error":
        variants = [
            multiline(f"whille {name} < {num}:", f"    print({name})"),
            f"pritn(\"{name}\")",
            f"defin {name}():",
            f"returnr {name}",
            f"iif {name} == {num}:",
    ...
    if error_type == "missing_colon":
        variants = [
            ...
            multiline(f"for {name} in range({num})", f"    total += {name}", "print(total)"),
            multiline(f"while {name} <= {num}", f"    {name} += 1", f"print({name})"),
            multiline(f"def {name}()", f"    return {name}", f"print({name})"),
    ...
    if error_type == "missing_parenthesis":
        variants = [
            multiline(f"print(\"{name}\"", f"if {name} > 0:", f"    print({name})"),
    ...
    if error_type == "indentation_error":
        variants = [
            multiline(f"if {name} > {num}:", f"    print({name})", "  print(\"done\")"),
            multiline(f"def {name}():", f"\tprint({name})", f"  return {name}"),
    ...
    if error_type == "unexpected_indent":
        variants = [
            ...
            multiline(f"    if {name} > {num}:", f"print({name})"),
    ...
    if error_type == "undefined_variable":
        variants = [
            ...
            multiline(f"if {name} > 0:", f"    result = {name} + 1", "print(result)"),
    ...
    if error_type == "wrong_loop_condition":
        variants = [
            ...
            multiline(f"while {name} < 0:", f"    print({name})", "print(\"done\")"),
...
def _make_entry(error_type: str, archetype: str = None, mark_confusion: bool = False) -> Dict[str, Any]:
    code, key, _, label = generate_code(error_type)

    if archetype:
        student_history = _student_history(archetype)
        hint_style = student_history["hint_style"]
    else:
        hint_style = random.choice(["socratic", "direct", "supportive", "silent", "nudge", "gentle"])
        student_history = {
            "past_errors": {"syntax_errors": random.randint(0, 3), "logic_errors": random.randint(0, 2), "type_errors": random.randint(0, 2)},
            "struggle_concepts": [] if error_type == "no_error" else [random.choice(["loops", "conditionals", "variables"])],
            "hint_style": hint_style,
        }

    # ── KEY FIX: use context-aware message instead of static ERROR_MESSAGES ──
    message = _build_training_message(error_type, code, hint_style)

    intervention_type, should_speak, tone, next_best_action, error_risk = _intervention_for(
        error_type, archetype or "confident_builder"
    )
    confidence = _choose_confidence(error_type)
    target = _make_target(
        error_type, cursor_after(code)["lineNumber"], cursor_after(code)["column"],
        message, intervention_type, should_speak, tone, next_best_action, confidence, error_risk
    )
    if mark_confusion:
...
def _make_rejection_examples(example: Dict[str, Any], error_type: str) -> List[Dict[str, Any]]:
    """
    Rejection examples teach the model that this code is NOT the confusable type.
    CRITICAL FIX: Keep the original student-facing message intact.
    Only mark the rejected_type and set low rejection_confidence.
    The message must remain a valid student-facing hint, not an internal label.
    """
    rejections = []
    rejected_types = CONFUSABLE_MAP.get(error_type, [])[:3]
    for rejected_type in rejected_types:
        rejection = copy.deepcopy(example)
        # ── DO NOT overwrite message or evidence with internal labels ──
        # The message stays as the correct student-facing hint for this error_type.
        # Only add the discrimination metadata that the training pipeline needs.
    ...
    for error_type, min_count in ERROR_MIN_COUNTS.items():
        ...
        if error_type == "incomplete_statement":
            for _ in range(200):
                ...
                # Use context-aware message for these too
                entry["TARGET"]["message"] = "Good start! Now what should happen inside this block? Remember to indent."
                entry["TARGET"]["diagnosis"][0]["evidence"] = entry["TARGET"]["message"]
...
def generate_critical_pair_examples() -> List[Dict[str, Any]]:
    """
    Critical pair examples teach the model to distinguish confusable error types.
    CRITICAL FIX: Do NOT override message with internal discrimination labels.
    The message must remain the correct student-facing hint for the actual error_type.
    """
    ...
    for a, b in CRITICAL_PAIRS:
        for error_type in (a, b):
            for _ in range(200):
                entry = _make_entry(error_type)
                # Mark the confusable pair for training awareness — metadata only
                entry["TARGET"]["confusable_with"] = b if error_type == a else a
                # ── DO NOT overwrite message — keep the student-facing hint intact ──
...
def main() -> None:
    print("Generating precision diagnostic examples for 21 error types...")
